...
function Carrousel({ seleccionado }) {
        ...
        async function chargeCarsExpo() {
            try {
                const response = await fetch(
                    `http://127.0.0.1:8000/api/coches_expo/?expo=${seleccionado}`
                );
    ...
    return (
        <div className="w-full h-[400px] sm:h-[500px] md:h-[600px] flex justify-center items-center relative overflow-hidden">

            {/* Overlay */}
            <div
                className={`fixed inset-0 bg-black/60 transition-opacity duration-300 ${
                    showInfo ? 'opacity-100 z-40' : 'opacity-0 pointer-events-none z-0'
                }`}
                onClick={closeMenu}
            />
                ...
                <div className="p-6 flex flex-col items-center w-full">
                    ...
                    {currentCar && (
                        <>
                            ...
                            {/* Info */}
                            <div className="text-center w-full max-w-md text-blue-950 flex flex-col mb-4">
                                <h3 className="text-xl font-bold uppercase">
                                    {currentCar.name}
                                </h3>
                                <p className="text-gray-600 text-xs">
                                    {currentCar.description || "Sin descripción."}
                                </p>
                            </div>
                            ...
                            {currentCar.images && currentCar.images.length > 0 && (
                                <div className="w-full max-w-sm mt-2 px-2">
                                    <h4 className="text-[10px] font-bold text-gray-400 mb-2 uppercase tracking-widest">
                                        Toca para ampliar
                                    </h4>

                                    <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar w-full">

                                        {/* imagen principal en miniaturas */}
                                        <img
                                            src={`http://127.0.0.1:8000${currentCar.image}`}
                                            onClick={() => setTempImage(currentCar.image)}
                                            className={`h-20 w-20 flex-shrink-0 object-cover rounded-lg border-2 cursor-pointer transition-all ${
                                                tempImage === currentCar.image
                    ...
                    <div
                        className="mt-2 w-10 h-1 bg-gray-200 rounded-full cursor-pointer"
                        onClick={closeMenu}
                    />
                </div>
            </div>
            ...
            {/* contador */}
            <div className="absolute top-4 right-4 bg-black/50 backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs z-30">
