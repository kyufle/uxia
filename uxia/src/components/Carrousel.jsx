...
function Carrousel({ seleccionado }) {
    ...
    useEffect(() => {
        async function chargeCarsExpo() {
            try {
                const response = await fetch(`http://uxiaweb2.ieti.site/api/coches_expo/?expo=${seleccionado}`);
                    ...
                    {currentCar && (
                        <>
                            <div className="w-full max-w-sm mb-4">
                                <img 
                                    src={`http://uxiaweb2.ieti.site${tempImage || currentCar.image}`} 
                                    alt={currentCar.name} 
                            ...
                            {currentCar.images && currentCar.images.length > 0 && (
                                <div className="w-full max-w-sm mt-2 px-2">
                                    ...
                                    <div className="flex flex-row gap-2 overflow-x-auto pb-4 no-scrollbar">
                                        <img 
                                            src={`http://uxiaweb2.ieti.site${currentCar.image}`}
                                        ...
                                        {currentCar.images.map((imgUrl, idx) => (
                                            <img 
                                                key={idx}
                                                src={`http://uxiaweb2.ieti.site${imgUrl}`}
                                                alt={`Vista ${idx}`}
            ...
            <div className="flex justify-center items-center w-full h-full">
                {cars.map((photo, index) => (
                    indexPhoto === index && (
                        <img
                            key={photo.id || index}
                            src={`http://uxiaweb2.ieti.site${photo.image}`}
                            alt={photo.name}
